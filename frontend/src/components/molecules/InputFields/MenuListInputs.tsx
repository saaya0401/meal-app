            ...
            {menus.map((menu, index) => (
                <Flex align="center" gap={{base: 2, md: 5}} flexDirection={{base: "column", md: "row"}}>
                    <Flex key={index} gap={{base:2 , md: 5}} align="center">
                        <Input type="text" fontSize={{base: "sm" , md: "xl"}} placeholder="例：かぼちゃ" bg="white" w={{base: "170px", md: "280px"}} value={menu.name} onChange={(e) => onChange(index, "name", e.target.value)}/>
                        <Flex align="flex-end" gap={{base:2 , md: 2}}>
                            <Input w={{base: "20px", md: "65px"}} fontSize={{base: "sm" , md: "xl"}} bg="white" type="text" value={ menu.amount } onChange={(e) => onChange(index, "amount", e.target.value)}/>
