...
export const MenuListInputs: FC = memo(() => {
    const { menus, menuChange, menuAdd,} = useLogsContext();
    return (
            ...
            {menus.map((menu, index) => (
                <Flex  key={index} align="center" gap={{base: 2, md: 5}} flexDirection={{base: "column", md: "row"}}>
                    <Flex gap={{base:2 , md: 5}} align="center">
                        <Input id={`menu-input-${index}`} type="text" fontSize={{base: "sm" , md: "xl"}} placeholder="例：かぼちゃ" bg="white" w={{base: "170px", md: "280px"}} value={menu.name} onChange={(e) => {menuChange(index, "name", e.target.value)}}/>
                        <Flex align="flex-end" gap={{base:2 , md: 2}}>
                            <Input id={`menu-amount-${index}`} w={{base: "20px", md: "65px"}} fontSize={{base: "sm" , md: "xl"}} bg="white" type="text" value={ menu.amount } onChange={(e) => menuChange(index, "amount", e.target.value)}/>
